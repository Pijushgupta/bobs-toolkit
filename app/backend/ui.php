...
class ui {
	...
	public static function activate() {
		if (current_user_can('manage_options')) {
			add_action('admin_menu', array('bobsToolkit\backend\ui', 'create'));
			add_action('admin_enqueue_scripts', array('bobsToolkit\backend\ui', 'addAssets'));
		}
	}

	public static function create() {
		add_menu_page(
			__(TOOLKIT_ADMIN_MENU_TITLE, TOOLKIT_TEXT_DOMAIN),
			__(TOOLKIT_ADMIN_MENU_NAME, TOOLKIT_TEXT_DOMAIN),
			'manage_options',
			TOOLKIT_SLUG,
			array('bobsToolkit\backend\ui', 'render'),
	...
	public static function render() {
		...
		$assetPath 	= TOOLKIT_REL . '/app/backend/assets';
		...
		printf(
			...
			$assetPath,
			$dashboardLang,
			TOOLKIT_HTML_ROOT_ID
		);
	}

	public static function addAssets($hook) {
		if ($hook != 'toplevel_page_' . TOOLKIT_SLUG) return;
		if (file_exists(TOOLKIT_ABS . 'app/backend/assets/dist/main.js')) {
			wp_enqueue_script('toolkit-vue-script', TOOLKIT_REL . '/app/backend/assets/dist/main.js', array(), '1.0.0', true);
		}
		if (file_exists(TOOLKIT_ABS . 'app/backend/assets/dist/main.css')) {
			wp_enqueue_style('toolkit-style', TOOLKIT_REL . '/app/backend/assets/dist/main.css', array(), '1.0.0');
		}
	}
}
